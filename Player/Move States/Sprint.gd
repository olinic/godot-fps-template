...
func get_next_state(input_dir: Vector2) -> Optional:
	if Input.is_action_just_pressed("jump"):
		_next_state = Optional.of(
				Jump.new(_player, _player_is_on_floor, _velocity, self._speed, Sprint.new(_player, _player_is_on_floor)))
	elif !_player_is_on_floor.call():
		_next_state = Optional.of(
				Fall.new(_player, _player_is_on_floor, _velocity, self._speed, Sprint.new(_player, _player_is_on_floor)))
